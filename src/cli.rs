...
impl CliController {
    ...
    fn run_direct(&self, message: &str) -> ExitCode {
        ...
        // Step 2: Compile using compiler pipeline
        let formatted_commit = match compile(message) {
            Ok(output) => output,
            Err(e) => {
        ...
        // Step 3: Preview formatted output
        println!("=== Preview ===\n");
        ...
        match self.commit_executor.execute(&formatted_commit) {
            Ok(result) => {
